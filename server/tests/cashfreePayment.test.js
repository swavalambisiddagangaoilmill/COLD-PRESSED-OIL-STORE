...
const original = { fetch: global.fetch, productFind: Product.find, userFind: User.findById, checkoutCreate: PaymentCheckout.create, checkoutFind: PaymentCheckout.findOne, checkoutUpdate: PaymentCheckout.updateOne, orderFind: Order.findOne };
...
test.afterEach(() => { global.fetch = original.fetch; Product.find = original.productFind; User.findById = original.userFind; PaymentCheckout.create = original.checkoutCreate; PaymentCheckout.findOne = original.checkoutFind; PaymentCheckout.updateOne = original.checkoutUpdate; Order.findOne = original.orderFind; });
...
test("duplicate verified payment resolves to the existing order", async () => {
  ...
  Order.findOne = async () => ({ _id: "existing-order" });
