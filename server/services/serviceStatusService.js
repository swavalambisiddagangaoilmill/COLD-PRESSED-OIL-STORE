...
function configurationSnapshot() {
  return {
    cashfree: hasAll([env.cashfree.clientId, env.cashfree.clientSecret])
...
async function checkCashfree() {
  if (!hasAll([env.cashfree.clientId, env.cashfree.clientSecret])) return status("cashfree", "not_configured", "Not configured.");
  try {
    const response = await fetchWithTimeout(`${env.cashfree.baseUrl}/orders/service_status_probe`, { headers: { "x-api-version": env.cashfree.apiVersion, "x-client-id": env.cashfree.clientId, "x-client-secret": env.cashfree.clientSecret } });
    if (response.status === 404) return status("cashfree", "online", `Operational; ${env.cashfree.environment} credentials accepted.`);
    if (response.status === 401 || response.status === 403) return status("cashfree", "offline", "Authentication failed.");
    return status("cashfree", "degraded", "Provider reachable; credentials could not be conclusively verified.");
  } catch {
