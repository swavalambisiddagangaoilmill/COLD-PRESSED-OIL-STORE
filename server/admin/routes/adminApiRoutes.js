...
router.put("/inventory/:id", requireAdminPermission("inventory.update"), productIdValidator, [body("mode").isIn(["add", "reduce", "set"]).withMessage("Valid stock update mode is required."), body("quantity").isFloat({ min: 0 }).withMessage("Stock litres must be zero or more."), body("variantId").optional().isMongoId().withMessage("Valid variant id is required."), body().custom((value) => value.mode === "set" || Number(value.quantity) > 0).withMessage("Add or reduce litres must be greater than zero.")], validate, controller.inventoryUpdate);
