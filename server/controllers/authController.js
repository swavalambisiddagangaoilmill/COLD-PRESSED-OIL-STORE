...
export const requestOtp = asyncHandler(async (req, res) => sendSuccess(res, 200, "WhatsApp verification code sent", await requestAuthOtp(req.body, req)));
export const verifyOtp = asyncHandler(async (req, res) => { const result = await verifyAuthOtp(req.body, req); setSession(res, result.token, result.refreshToken); sendSuccess(res, 200, "Mobile number verified", { user: result.user }); });
