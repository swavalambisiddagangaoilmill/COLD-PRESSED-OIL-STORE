...
import { addAddress, authenticateWithGoogle, deleteAddress, getSecuritySummary, logoutUser, refreshUserSession, requestAuthOtp, revokeUserSession, updateAddress, updateUserProfile, verifyAuthOtp } from "../services/authService.js";
...
export const googleLogin = asyncHandler(async (req, res) => { const result = await authenticateWithGoogle(req.body.credential, req); setSession(res, result.token, result.refreshToken); sendSuccess(res, 200, "Google sign-in successful", { user: result.user }); });
