...
export const getPaymentStatus = asyncHandler(async (req, res) => sendSuccess(res, 200, "Payment status synchronized", await getPaymentCheckoutStatus(req.user._id, req.params.cashfreeOrderId)));
export const getOrderConfirmation = asyncHandler(async (req, res) => sendSuccess(res, 200, "Order confirmation fetched", { order: await getCheckoutConfirmation(req.user._id, req.params.checkoutSessionId) }));
