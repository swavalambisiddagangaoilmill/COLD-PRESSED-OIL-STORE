...
function AdminSettingsExtrasContent() {
  ...
  const load = async () => {
    ...
    const types = Array.isArray(prefData?.types) ? prefData.types.filter(Boolean) : [];
    setPrefs(types);
    setEnabled(Object.fromEntries(types.map((item) => [item.type, item.enabled])));
    setSessions({ ...sessionData, active: Array.isArray(sessionData?.active) ? sessionData.active.filter(Boolean) : [] });
  };
  useEffect(() => { load(); }, []);
  ...
  const revoke = async (ids) => {
    ...
    if (result) setSessions((current) => current ? { ...current, active: (current.active || []).filter((session) => session && !ids.includes(session.sessionId)) } : current);
  };
  const grouped = (Array.isArray(prefs) ? prefs : []).filter(Boolean).reduce((acc, item) => ({ ...acc, [item.category]: [...(acc[item.category] || []), item] }), {});
...
function ShiprocketSwitch() {
  const { showToast } = useToast();
  const [settings, setSettings] = useState(null);
  const [saving, setSaving] = useState(false);
  useEffect(() => { adminApi.settings().then((data) => setSettings(data.settings)).catch(() => setSettings(null)); }, []);
  if (!settings) return null;
  const save = async (enabled) => {
    setSaving(true);
    try {
      const data = await adminApi.saveSettings({ ...settings, shiprocketEnabled: enabled });
      setSettings(data.settings);
      showToast(`Shiprocket ${enabled ? "enabled" : "disabled"}.`, "success");
    } catch (error) {
      showToast(error.message || "Shiprocket setting could not be saved.", "error");
    } finally { setSaving(false); }
  };
  return <section className="grid gap-4 rounded-xl border border-[var(--admin-border)] bg-white p-5 shadow-sm xl:col-span-2"><div><h2 className="font-bold">Shiprocket</h2><p className="mt-1 text-sm text-ink/55">Production shipping safety switch. Disabling it blocks shipping quotes and checkout; it never creates fallback rates.</p></div><label className="flex items-center gap-3 text-sm font-bold"><input type="checkbox" checked={settings.shiprocketEnabled !== false} disabled={saving} onChange={(event) => save(event.target.checked)} />{settings.shiprocketEnabled !== false ? "ON" : "OFF"}</label></section>;
}

export default function AdminSettingsExtras() {
  return <><ShiprocketSwitch /><AdminSettingsExtrasContent /></>;
}
