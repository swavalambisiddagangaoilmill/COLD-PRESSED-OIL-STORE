...
function ProductEditor({ open, onClose, product, categories, onSaved }) {
  ...
  const validate = () => {
    const next = {
      ...
      variants: form.variants?.some((variant) => variant.isActive !== false) && form.variants.every((variant) => /^(\d+(?:\.\d+)?)\s*(ml|l|litres?|liters?)$/i.test(variant.size?.trim()) && Number(variant.price) > 0 && Number(variant.mrp) >= Number(variant.price) && Number.isFinite(Number(variant.stock)) && Number(variant.stock) >= 0 && variant.images?.length) && new Set(form.variants.map((variant) => variant.size.trim().toLowerCase())).size === form.variants.length ? "" : "Add at least one active variant. Every variant needs a unique volume such as 500ml, 1L, or 16.5L, valid price/MRP, non-negative stock litres, and at least one image.",
    };
    setErrors(next);
...
export function InventoryPage() {
  ...
  const items = (data?.items || []).flatMap((product) => (product.variants || []).map((variant) => ({ product, variant, stock: Number(variant.stock || 0) }))).filter((item) => filter === "All" || status(item.stock) === filter);
  const saveRow = (product) => updateItemList(setData, product._id, product);
  return <><AdminPageHeader title="Inventory" description="Update each variant's authoritative stock in litres." /><AdminFilters><AdminSelect label="Stock Status" value={filter} onChange={(e) => setFilter(e.target.value)}>{["All", "In Stock", "Low Stock", "Out of Stock"].map((item) => <option key={item}>{item}</option>)}</AdminSelect></AdminFilters><State loading={loading} error={error} empty={!items.length} />{items.length ? <AdminTable columns={["Product", "Variant", "SKU", "Available Litres", "Stock Status", "Actions"]} rows={items.map(({ product, variant, stock }) => <tr key={`${product._id}:${variant._id}`}><Cell>{product.title}</Cell><Cell>{variant.size}</Cell><Cell>{variant.sku || "-"}</Cell><Cell>{stock}L available</Cell><Cell><AdminBadge>{status(stock)}</AdminBadge></Cell><Cell><div className="flex gap-2"><AdminButton variant="secondary" onClick={() => setEditing({ product, variant, mode: "add", quantity: 1 })}>Add Litres</AdminButton><AdminButton variant="secondary" onClick={() => setEditing({ product, variant, mode: "reduce", quantity: 1 })}>Reduce Litres</AdminButton><AdminButton variant="secondary" onClick={() => setEditing({ product, variant, mode: "set", quantity: stock })}>Set Exact</AdminButton></div></Cell></tr>)} /> : null}<StockModal state={editing} onClose={() => setEditing(null)} onSaved={saveRow} /></>;
}
...
function StockModal({ state, onClose, onSaved }) {
  ...
  const current = Number(state.variant?.stock ?? state.product.stock ?? 0);
