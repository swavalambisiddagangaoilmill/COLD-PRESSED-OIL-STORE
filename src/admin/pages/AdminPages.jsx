...
function OfferForm({ open, offer, categories, products, onClose, onSaved }) {
  ...
  const save = async () => {
    ...
    if (!form.name.trim() || Number(form.discountValue) <= 0 || Number(form.discountValue) > 100 || !allowed || new Date(form.endDate) <= new Date(form.startDate)) return;
    ...
    const result = await run("offer:save", () => offer?._id ? adminApi.updateOffer(offer._id, payload) : adminApi.createOffer(payload), "Offer saved.");
    if (result?.offer) { onSaved(result.offer); window.dispatchEvent(new CustomEvent("ss-oil-mill-promotions-changed")); onClose(); }
  };
  return <AdminModal title={offer?._id ? "Edit Offer" : "Create Offer"} open={open} onClose={onClose} footer={<AdminButton loading={pending["offer:save"]} onClick={save}>{offer?._id ? "Save Offer" : "Create Offer"}</AdminButton>}><div className="grid gap-4"><div className="grid gap-4 md:grid-cols-2"><AdminInput label="Offer name/title" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} /><AdminInput label="Discount percentage" type="number" min="0.01" max="100" value={form.discountValue} onChange={(e) => setForm({ ...form, discountValue: e.target.value })} /><AdminInput label="Start date/time" type="datetime-local" value={form.startDate} onChange={(e) => setForm({ ...form, startDate: e.target.value })} /><AdminInput label="End date/time" type="datetime-local" value={form.endDate} onChange={(e) => setForm({ ...form, endDate: e.target.value })} /></div><AdminTextarea label="Description" value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} /><div><p className="mb-2 text-sm font-bold">Offer applies to:</p><div className="flex flex-wrap gap-3">{[["CATEGORY","Categories"],["VARIANT","Specific variants"],["CUSTOM","Custom selection"]].map(([value,label]) => <label key={value} className="flex items-center gap-2 rounded-lg bg-linen px-3 py-2 text-sm font-semibold"><input type="radio" checked={form.targetType === value} onChange={() => setForm({ ...form, targetType: value })} />{label}</label>)}</div></div>{form.targetType !== "VARIANT" && <div><p className="mb-2 text-sm font-bold">Categories</p><div className="grid gap-2 md:grid-cols-2">{categories.map((category) => <label key={category._id} className="flex gap-2 text-sm"><input type="checkbox" checked={form.categories.map(String).includes(String(category._id))} onChange={() => toggle("categories", category._id)} />{category.name}</label>)}</div></div>}{form.targetType === "CUSTOM" && <div><p className="mb-2 text-sm font-bold">Products</p><div className="grid max-h-40 gap-2 overflow-auto md:grid-cols-2">{products.map((product) => <label key={product._id} className="flex gap-2 text-sm"><input type="checkbox" checked={form.products.map(String).includes(String(product._id))} onChange={() => toggle("products", product._id)} />{product.title}</label>)}</div></div>}{form.targetType !== "CATEGORY" && <div><p className="mb-2 text-sm font-bold">Variants</p><div className="grid max-h-52 gap-2 overflow-auto">{products.flatMap((product) => (product.variants || []).map((variant) => <label key={`${product._id}:${variant._id}`} className="flex gap-2 text-sm"><input type="checkbox" checked={form.variants.some((item) => String(item.product?._id || item.product) === String(product._id) && String(item.variant) === String(variant._id))} onChange={() => toggleVariant(product, variant)} />{product.title} — {variant.size}</label>))}</div></div>}<Toggle label="Active" checked={form.isActive} onChange={(value) => setForm({ ...form, isActive: value })} /><div className="rounded-xl bg-leaf/10 p-4 text-sm"><p className="font-bold">{form.discountValue || 0}% OFF</p><p>Selected targets: {form.targetType === "CATEGORY" ? form.categories.length : form.targetType === "VARIANT" ? form.variants.length : targets}</p>{sample && <p>Example: {money(sample.price)} → {money(preview)}</p>}</div></div></AdminModal>;
}
