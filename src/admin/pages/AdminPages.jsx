...
function GalleryForm({ open, image, onClose, onSaved }) {
  const { pending, run } = useAdminAction();
  const { data: serviceData } = useAdminData(adminApi.serviceStatus);
  const uploadAvailable = serviceData?.services?.cloudinary?.available !== false;
  const uploadMessage = serviceData?.services?.cloudinary?.message || "Image uploads are temporarily unavailable.";
  const [form, setForm] = useState(image || { title: "", description: "", image: null, isVisible: true });
  useEffect(() => setForm(image || { title: "", description: "", image: null, isVisible: true }), [image, open]);
  const imageUrl = form.image?.url || form.image || "";
  const upload = async (file) => { if (!uploadAvailable) return; const data = await run("gallery:image", () => adminApi.uploadImage(file), "Image uploaded."); if (data) setForm((current) => ({ ...current, image: data.image || data })); };
  const save = async () => { const result = await run("gallery:save", () => adminApi.saveGalleryImage(form, image?._id), "Gallery image saved."); if (result?.image) { onSaved(result.image); onClose(); } };
  return <AdminModal title={image?._id ? "Edit Gallery Image" : "Add Gallery Image"} open={open} onClose={onClose} footer={<AdminButton disabled={!imageUrl} loading={pending["gallery:save"]} onClick={save}>Save Image</AdminButton>}><div className="grid gap-4"><div>{imageUrl && <img src={imageUrl} alt="" className="mb-3 aspect-[4/3] w-full max-w-sm rounded-xl object-cover" />}<label title={uploadAvailable ? "" : uploadMessage} className={`inline-flex rounded-lg border border-[var(--admin-border)] px-3 py-2 text-sm font-bold ${uploadAvailable ? "cursor-pointer" : "cursor-not-allowed opacity-50"}`}>Upload Image<input type="file" accept="image/*" disabled={!uploadAvailable} className="hidden" onChange={(e) => e.target.files?.[0] && upload(e.target.files[0])} /></label></div><AdminInput label="Title" value={form.title || ""} onChange={(e) => setForm({ ...form, title: e.target.value })} /><AdminTextarea label="Short Description" value={form.description || ""} onChange={(e) => setForm({ ...form, description: e.target.value })} /><Toggle label="Visible on website" checked={form.isVisible !== false} onChange={(value) => setForm({ ...form, isVisible: value })} /></div></AdminModal>;
}
...
export function GalleryPage() {
  ...
  const [editing, setEditing] = useState(null);
  const items = data?.items || [];
  const saveRow = (image) => setData((current) => current ? { ...current, items: current.items?.some((item) => item._id === image._id) ? current.items.map((item) => item._id === image._id ? { ...item, ...image } : item) : [...(current.items || []), image].sort((a, b) => (a.sortOrder || 0) - (b.sortOrder || 0)) } : current);
  ...
  const toggle = async (image) => { const result = await run(`gallery:toggle:${image._id}`, () => adminApi.saveGalleryImage({ title: image.title, description: image.description, image: image.image, sortOrder: image.sortOrder, isVisible: !image.isVisible }, image._id), image.isVisible ? "Image hidden." : "Image visible."); if (result?.image) updateItemList(setData, image._id, result.image); };
  ...
  return <><AdminPageHeader title="Gallery" description="Manage homepage gallery images." action={<AdminButton onClick={() => setEditing({})}><Plus size={16} />Add Image</AdminButton>} /><State loading={loading} error={error} empty={!items.length} title="No gallery images yet." description="Upload images to show in the homepage gallery." action={<AdminButton onClick={() => setEditing({})}>Add Image</AdminButton>} />{items.length ? <AdminTable columns={["Preview", "Title", "Description", "Status", "Order", "Actions"]} rows={items.map((item, index) => <tr key={item._id}><Cell>{item.image?.url ? <img src={item.image.url} alt="" className="h-14 w-20 rounded-lg object-cover" /> : "-"}</Cell><Cell className="font-bold">{item.title || "Untitled"}</Cell><Cell><span className="block max-w-xs truncate">{item.description || "-"}</span></Cell><Cell><AdminBadge>{item.isVisible ? "Active" : "Disabled"}</AdminBadge></Cell><Cell>{index + 1}</Cell><Cell><div className="flex flex-wrap gap-2"><AdminButton variant="secondary" disabled={index === 0} loading={pending["gallery:reorder"]} onClick={() => move(index, -1)}><ArrowUp size={14} /></AdminButton><AdminButton variant="secondary" disabled={index === items.length - 1} loading={pending["gallery:reorder"]} onClick={() => move(index, 1)}><ArrowDown size={14} /></AdminButton><AdminButton variant="secondary" onClick={() => setEditing(item)}>Edit</AdminButton><AdminButton variant="secondary" loading={pending[`gallery:toggle:${item._id}`]} onClick={() => toggle(item)}>{item.isVisible ? <EyeOff size={14} /> : <Eye size={14} />}</AdminButton><AdminButton variant="danger" loading={pending[`gallery:delete:${item._id}`]} onClick={() => remove(item)}><Trash2 size={14} /></AdminButton></div></Cell></tr>)} /> : null}<GalleryForm open={Boolean(editing)} image={editing?._id ? editing : null} onClose={() => setEditing(null)} onSaved={saveRow} /></>;
}
