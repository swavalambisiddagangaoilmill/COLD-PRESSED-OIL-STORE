...
export default function FulfillmentPage() {
  ...
  const selectable = useMemo(() => orders.filter((order) => order.orderStatus === "confirmed" && !order.awbCode && !order.shipmentCreationStartedAt), [orders]);
  ...
  return <>
    <AdminPageHeader title="Confirmed Orders" description="Prepare confirmed orders and submit eligible shipments to Shiprocket." action={<AdminButton variant="secondary" loading={exporting} onClick={download}><Download size={16} />Download Order List</AdminButton>} />
    <div className="mb-4 grid gap-3 rounded-xl border border-ink/10 bg-white p-3 shadow-sm sm:grid-cols-2 xl:grid-cols-[minmax(240px,1fr)_220px_220px]">
      ...
      <AdminSelect label="Fulfillment status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="all">All statuses</option><option value="pending">Confirmed</option><option value="failed">Shipment failed</option><option value="requires_details">Needs details</option><option value="ready_for_pickup">Waiting for pickup</option><option value="picked_up">Picked up</option><option value="in_transit">In transit</option><option value="out_for_delivery">Out for delivery</option><option value="delivered">Delivered</option></AdminSelect>
    ...
    <div className="mb-4 flex flex-col gap-3 rounded-xl border border-ink/10 bg-white p-3 shadow-sm sm:flex-row sm:items-center sm:justify-between">
      ...
      <AdminButton className="h-11 w-full sm:w-auto" disabled={!selected.length} loading={processing} onClick={() => submit()}><PackageCheck size={17} />Ready for Shipment</AdminButton>
    </div>
    ...
    {!loading && !error && orders.length > 0 && <>
      <div className="grid gap-3 lg:hidden">{orders.map((order) => { const eligible = selectable.some((item) => item._id === order._id); return <article key={order._id} className="rounded-xl border border-ink/10 bg-white p-4 shadow-sm"><div className="flex items-start gap-3"><SelectionBox checked={selected.includes(order._id)} onChange={() => eligible && toggle(order._id)} label={`Select order ${order._id}`} /><div className="min-w-0 flex-1"><p className="break-all text-sm font-bold">{order._id}</p><p className="mt-1 text-sm text-ink/55">{order.user?.name || order.shippingAddress?.fullName || "Customer"}</p></div><AdminBadge>{label(order.shippingStatus)}</AdminBadge></div><div className="mt-4 grid grid-cols-2 gap-3 text-sm"><div><p className="text-xs font-bold uppercase text-ink/40">Confirmed</p><p className="mt-1">{dateTime(order.confirmedAt)}</p></div><div><p className="text-xs font-bold uppercase text-ink/40">Total</p><p className="mt-1 font-bold">{money(order.totalAmount)}</p></div><div className="col-span-2"><p className="text-xs font-bold uppercase text-ink/40">Products</p><div className="mt-1"><ProductList products={order.products} /></div></div><div><p className="text-xs font-bold uppercase text-ink/40">Payment</p><p className="mt-1">{label(order.paymentStatus)}</p></div><div><p className="text-xs font-bold uppercase text-ink/40">AWB</p><p className="mt-1 break-all">{order.awbCode || "Not assigned"}</p></div></div>{order.shippingFailureReason && <p className="mt-3 rounded-lg bg-red-50 p-3 text-sm text-red-700">{order.shippingFailureReason}</p>}</article>; })}</div>
      <div className="hidden overflow-x-auto rounded-xl border border-ink/10 bg-white shadow-sm lg:block"><table className="min-w-[1280px] divide-y divide-ink/10 text-sm"><thead className="bg-linen/70"><tr>{["", "Order", "Customer", "Order date", "Confirmed", "Products", "Total", "Payment", "Fulfillment", "Shiprocket", "AWB", "Updated"].map((heading) => <th key={heading} className="whitespace-nowrap px-3 py-3 text-left text-xs font-bold uppercase tracking-[0.08em] text-ink/50">{heading}</th>)}</tr></thead><tbody className="divide-y divide-ink/10">{orders.map((order) => { const eligible = selectable.some((item) => item._id === order._id); return <tr key={order._id}><td className="px-3 py-3"><SelectionBox checked={selected.includes(order._id)} onChange={() => eligible && toggle(order._id)} label={`Select order ${order._id}`} /></td><td className="max-w-44 break-all px-3 py-3 font-bold">{order._id}</td><td className="px-3 py-3">{order.user?.name || order.shippingAddress?.fullName || "Customer"}</td><td className="whitespace-nowrap px-3 py-3">{dateTime(order.createdAt)}</td><td className="whitespace-nowrap px-3 py-3">{dateTime(order.confirmedAt)}</td><td className="min-w-52 px-3 py-3"><ProductList products={order.products} /></td><td className="whitespace-nowrap px-3 py-3 font-bold">{money(order.totalAmount)}</td><td className="px-3 py-3"><AdminBadge>{label(order.paymentStatus)}</AdminBadge></td><td className="px-3 py-3"><AdminBadge>{label(order.orderStatus)}</AdminBadge></td><td className="px-3 py-3"><AdminBadge>{label(order.shippingStatus)}</AdminBadge></td><td className="max-w-40 break-all px-3 py-3">{order.awbCode || "-"}</td><td className="whitespace-nowrap px-3 py-3">{dateTime(order.updatedAt)}</td></tr>; })}</tbody></table></div>
    </>}
  </>;
}
